...
void tpch_q16_driver::submit(void* disp, memObject_t*) {
    ...
    packet_t* partsupp_scan_packet =
        partsupp_scan(tpch_tables[TPCH_TABLE_PARTSUPP].db);
    packet_t* supplier_scan_packet =
        supplier_scan(tpch_tables[TPCH_TABLE_SUPPLIER].db);
