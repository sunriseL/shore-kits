  ...
  { // make gotos safe
    ...
    //BEGIN FRAME4
    {
      ...
      if(!type_is_market){
        ...
        TRACE( TRACE_TRX_FLOW, "App: %d TO:tr-add-tuple (%ld), (%ld), (%s), (%ld), (%ld), (%ld) \n", xct_id, trade_id,  ptoin._trade_type_id, symbol, ptoin._trade_qty, requested_price, broker_id);
...
w_rc_t ShoreTPCEEnv::xct_trade_update(const int xct_id, trade_update_input_t& ptuin)
{
...

  {
    ...
    int max_trades = 20; //instead of ptuin._max_trades which comes from the input
