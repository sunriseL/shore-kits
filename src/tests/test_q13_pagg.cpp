...
struct int_desc_key_extractor_t : public key_extractor_t {
    virtual int extract_hint(const char* tuple_data) {
        int result;
        memcpy(&result, tuple_data, sizeof(result));
        return -result;
    }
...
struct q13_count_aggregate_t : public tuple_aggregate_t {
    ...
    virtual void aggregate(char* agg_data, const tuple_t &) {
        int count;
        memcpy(&count,
               agg_data + offsetof(key_count_tuple_t, COUNT),
               sizeof(count));
        count++;
        memcpy(agg_data + offsetof(key_count_tuple_t, COUNT),
               &count,
               sizeof(count));
    }
...
packet_t* customer_scan(Db* tpch_customer) {
    struct customer_tscan_filter_t : public tuple_filter_t {
        ...
        virtual void project(tuple_t &d, const tuple_t &s) {
            memcpy(d.data,
                   s.data + offsetof(tpch_customer_tuple, C_CUSTKEY),
                   sizeof(int));
        }
...
struct order_tscan_filter_t : public tuple_filter_t {
    ...
    virtual bool select(const tuple_t &input) {
        tpch_orders_tuple order;
        memcpy(&order, input.data, sizeof(order));
    ...
    virtual void project(tuple_t &d, const tuple_t &s) {
        // project C_CUSTKEY
        memcpy(d.data,
               s.data + offsetof(tpch_orders_tuple, O_CUSTKEY),
               sizeof(int));
    }
    ...
    for(int i=0; i < 10; i++) {
        ...
        struct q13_join_t : public tuple_join_t {
            struct right_key_extractor_t : public key_extractor_t {
                virtual int extract_hint(const char* tuple_data) {
                    int hint;
                    memcpy(&hint,
                           tuple_data + offsetof(key_count_tuple_t, KEY),
                           sizeof(int));
                    return hint;
                }
            ...
            struct left_key_extractor_t : public key_extractor_t {
                virtual int extract_hint(const char* tuple_data) {
                    int hint;
                    memcpy(&hint, tuple_data, sizeof(int));
                    return hint;
                }
                              ...
                              const tuple_t &right)
            {
                ...
                memcpy(dest.data,
                       right.data + offsetof(key_count_tuple_t, COUNT),
                       sizeof(int));
            }
        ...
        struct q13_key_extract_t : public key_extractor_t {
            ...
            virtual int extract_hint(const char* tuple_data) {
                ...
                int count;
                memcpy(&count,
                       tuple_data + offsetof(key_count_tuple_t, COUNT),
                       sizeof(int));
                return count;
            }
